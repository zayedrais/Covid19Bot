...
function tryDotNetSDK() {
    ...
    try {
        require(`dotnet-sdk-${version}/dist/find.js`);
    }
    catch (_a) {
        tryDotNet();
    }
...
function tryDotNet() {
    ...
    try {
        require(`${common_1.dotnetPackageName}/find.js`);
    }
    catch (_a) {
        // run the dotnet package
        debug(`Trying user dotnet package: ${common_1.installationPath}`);
        try {
            require(`${common_1.installationPath}/find.js`);
        }
        catch (_b) {
            ...
            require("child_process").spawn(process.execPath, [`${__dirname}/app.js`, "--force"], { stdio: verbose ? [process.stdin, process.stderr, process.stderr] : 'ignore' }).
                on("exit", (code, signal) => code == 0 ? require(`${common_1.installationPath}/find.js`) : process.exit(console.error("Unable to install/use dotnet framework.") || code));
        }
    }
}
